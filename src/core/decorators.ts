...
export function ValidateBody(schema: ZodSchema<any>): MethodDecorator {
  return (target: any, propertyKey: string | symbol, descriptor: PropertyDescriptor) => {
    ...
    console.log(`@ValidateBody: Attached schema metadata to method ${String(propertyKey)}`);
  };
}
  ...
  controllers.forEach((controllerClass) => {
    ...
    routes.forEach(({ path, method, handlerName }) => {
      ...
      if (typeof instance[handlerName] !== 'function') {
          console.error(`Handler ${String(handlerName)} not found on controller ${controllerClass.name}`);
          return;
      }
      const handler = instance[handlerName].bind(instance);

      // *** Explicitly retrieve the schema using the dedicated key from the method ***
      const validationSchema = Reflect.getMetadata(VALIDATION_SCHEMA_KEY, instance, handlerName) as ZodSchema<any> | undefined;
      ...
      const validationMiddleware = (req: Request, res: Response, next: NextFunction) => {
         ...
         console.log(`Validation check for: ${req.method} ${req.originalUrl} - Schema Found: ${!!validationSchema}`);
         ...
         try {
           console.log(`Attempting validation for ${req.method} ${req.originalUrl} with body:`, req.body);
           req.body = validationSchema.parse(req.body); // Validate and potentially transform
           console.log(`Validation SUCCESS for ${req.method} ${req.originalUrl}`);
           next();
         } catch (error) {
           console.error(`Validation FAILED for ${req.method} ${req.originalUrl}:`, error instanceof ZodError ? error.format() : error);
           if (error instanceof ZodError) {
             // Pass structured validation errors
             next(new ValidationError(error));
           } else {
             // Pass other unexpected errors (e.g., programming errors in schema)
              next(error instanceof AppError ? error : new BadRequestError('Invalid request body structure.'));
           }
         }
      };

      // Register the route with Express, ensuring validation middleware runs first
      console.log(`Registering route: ${method.toUpperCase()} ${fullPath} (Validation: ${!!validationSchema})`);
      appOrRouter[method](fullPath, validationMiddleware, async (req: Request, res: Response, next: NextFunction) => {
        try {
        ...
        descriptor.value = function (...args: any[]) {
            ...
            if (result instanceof Promise) {
                return result.then(resolvedValue => {
                    memoizationCache.set(cacheKey, { value: resolvedValue, expiry: now + ttlMs });
