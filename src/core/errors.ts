...
export class ValidationError extends AppError { // Specific error for validation failures from Zod
    constructor(errorDetails: ZodError) {
        super('Validation failed', 422, 'VALIDATION_ERROR', ValidationError.formatErrors(errorDetails));
      }

      // Custom method to format Zod errors
    private static formatErrors(errorDetails: ZodError): Record<string, string[]> {
