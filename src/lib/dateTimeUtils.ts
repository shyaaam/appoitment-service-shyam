...
export function getUTCDateTimeLocal(date: Date, hours: number, minutes: number, timezone: string): Date {
...
export function isSlotBooked(
  ...
  for (const appt of bookedAppointments) {
    ...
    const containsAppt =
       (isBefore(slotTimeUTC, appt.startTime) || isEqual(slotTimeUTC, appt.startTime)) &&
       (isAfter(slotEndTimeUTC, appt.endTime) || isEqual(slotEndTimeUTC, appt.endTime));
 ...
 * @param tz - The string to check.
 */
export function assertIsValidIANATimezone(tz: string): asserts tz is IANATimezone {
  if (!isValidIANATimezone(tz)) {
    // Provide a more specific error type if desired
    throw new Error(`Invalid or unrecognized IANA timezone identifier: "${tz}"`);
  }
}