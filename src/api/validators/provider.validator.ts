...
}).refine(data => data.start < data.end, { // Basic check: end time must be after start time
...
export const upsertScheduleSchema = z.object({
  ...
  weeklySchedule: z.record(daysOfWeek, dailyScheduleSchema).refine(
    val => Object.keys(val).length > 0, { message: "Weekly schedule cannot be empty" }
  ),
  appointmentDuration: z.number().int().positive('Appointment duration must be a positive integer').min(15, 'Appointment duration must be at least 15 minutes'),
